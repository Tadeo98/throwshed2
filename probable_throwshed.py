...
def main2(dem_path,point_layer_path,line_layer_path,throwshed_output_folder,throwshed_file,throwshed_mode,use_viewshed,
          use_lines,band_number,interpolation,EPSG,atmosphere_type,numerical_method,trajectory_segment_dimension,
          irregular_projectile,MCS_dist,cores,initial_height,gravitational_acceleration,initial_velocity,temperature,
          air_density,drag_to_mach,diameter,cross_sectional_area,mass,alpha_min,alpha_max,trajectory_segment_size,
          azimuth_min,azimuth_max,eyes_height,target_height,wall_height,wall_width,oscillation_frequency,
          oscillation_distance,peak_drag,peak_area,MCS_number,DEM_dev):
    global TOF, SRS, dds, db, da, dgt, dndv
    TOF = throwshed_output_folder
    ...
    if MCS_dist:
        for key, value in parameters.items():
            #this is to distinct between constant drag coefficient which can be changed due to standard deviation and list or string drag to mach function that cannot be changed
            if type(value[0]) is not str and type(value[0]) is not list:
                parameters[key] = np.random.normal(value[0],value[1],MCS_number)
            else:
                parameters[key] = [value[0]]*MCS_number
    else:
        for key, value in parameters.items():
            if type(value[0]) is not str and type(value[0]) is not list:
    ...
    for i in range(MCS_number):
        ...
        parameters.get("mass")[i],parameters.get("cross_sectional_area")[i],trajectory_segment_size,azimuth_min,azimuth_max,eyes_height,target_height,wall_height,
        ...
        air_density=parameters.get("air_density")[i])
    ...
    create_raster_file(throwshed_file, PTRA, gdal.GDT_Float32, dndv, [1,2])
...
def create_raster_file(raster_name, dem_array, GDT, no_data, bands):
    ...
    outds = gdal.GetDriverByName('GTiff').Create(TOF + "\\" + raster_name + ".tif", xsize=da[0].shape[1],
...
def main():
    ## PATHS
    dem_path = r"D:\School\STU_SvF_BA\Term11\Dizertacna_praca\case_study\Throwshed_article_unibe\data\dtm10km_10m.tif"  # path to DEM
    point_layer_path = r"D:\School\STU_SvF_BA\Term11\Dizertacna_praca\case_study\Throwshed_article_unibe\data\point.shp"  # path to point layer, can't be situated on raster bordering column or row (due to linear interpolation or the multiprocessing azimuth computatuions)
    line_layer_path = r"D:\School\STU_SvF_BA\Term11\Dizertacna_praca\Throwshed2\data\line\lines1.shp"  # path to line layer
    throwshed_output_folder = r"D:\School\STU_SvF_BA\Term11\Dizertacna_praca\case_study\Throwshed_article_unibe\data"  # path to folder, where the file will be saved
    throwshed_file = r"sierra_probable"  # name of output Probable Throwshed File
    ...
    initial_height = [1.7, 0.1] #initial height of projectile above DEM when shot [m]
    gravitational_acceleration = [9.810, 0.010] #gravitational acceleration [m/s^2]
    initial_velocity = [808.0, 5.0] #initial velocity of projectile when shot [m/s]
    temperature = [15.0, 5.0] #air temperature at shooting site [°C]
    air_density = [1.058, 0.050] #air density [kg/m^3]
    drag_to_mach = ["308 Sierra 168gr", 0.0] #aerodynamic drag coefficient of projectile (constant, list or drag table)
    diameter = [0.0078, 0.0001] #diameter of the projectile [m^2]
    ...
    mass = [0.0109, 0.0005] #projectile mass [kg]
    #other variables:
    ...
    trajectory_segment_size = 1  # distance step (length or width), at which trajectory's points will be saved and compared to DEM [m], None = adjusted to DEM resolution (cell's size), any float/int value = customized distance step (equal or less than raster resolution)
    azimuth_min = 0 #minimum of the azimuth range [°]
    azimuth_max = 360 #maximum of the azimuth range [°]
    ...
    main2(dem_path,point_layer_path,line_layer_path,throwshed_output_folder,throwshed_file,throwshed_mode,use_viewshed,
          use_lines,band_number,interpolation,EPSG,atmosphere_type,numerical_method,trajectory_segment_dimension,
          irregular_projectile,MCS_dist,cores,initial_height,gravitational_acceleration,initial_velocity,temperature,
          air_density,drag_to_mach,diameter,cross_sectional_area,mass,alpha_min,alpha_max,trajectory_segment_size,
          azimuth_min,azimuth_max,eyes_height,target_height,wall_height,wall_width,oscillation_frequency,
